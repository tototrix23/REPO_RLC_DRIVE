...
void impl_log_write(uint8_t mode,uint8_t color,char*module,  char* file,const char* func, uint16_t line, char *va_string)
{
    ...
    {
        case LOG_COLOR_ERROR:
            impl_printf( RTT_CTRL_TEXT_BRIGHT_RED);
            impl_printf( RTT_CTRL_BG_BLACK);
            snprintf(buffer_func,sizeof(buffer_func), "%12s","[ERROR] ");
            break;

        case LOG_COLOR_WARN:
            impl_printf( RTT_CTRL_BG_BLACK);
            impl_printf( RTT_CTRL_TEXT_BRIGHT_YELLOW);
            snprintf(buffer_func,sizeof(buffer_func), "%12s","[WARNING] ");
            break;

        case LOG_COLOR_INFO:
            impl_printf( RTT_CTRL_BG_BLACK);//RTT_CTRL_BG_BLACK);
            impl_printf( RTT_CTRL_TEXT_BRIGHT_CYAN);//RTT_CTRL_TEXT_BRIGHT_CYAN);
            snprintf(buffer_func,sizeof(buffer_func), "%12s","[INFO] ");
            break;

        case LOG_COLOR_DEBUG:
            impl_printf( RTT_CTRL_BG_BLACK);
            impl_printf( RTT_CTRL_TEXT_BRIGHT_GREEN);
            snprintf(buffer_func,sizeof(buffer_func), "%12s","[DEBUG] ");
            break;

        case LOG_COLOR_DEFAULT:
            impl_printf( RTT_CTRL_BG_BLACK);
            impl_printf( RTT_CTRL_TEXT_BRIGHT_WHITE);
            snprintf(buffer_func,sizeof(buffer_func), "%12s","[STD] ");
            break;

        default:
            impl_printf( RTT_CTRL_BG_BLACK);
            impl_printf( RTT_CTRL_TEXT_BRIGHT_WHITE);
            snprintf(buffer_func,sizeof(buffer_func), "%12s","[DEF] ");
            break;
    ...
    snprintf(buffer_func,sizeof(buffer_func), "[%03d:%02d:%02d:%03dms]",hours,minuts,seconds,ms);
    impl_printf(buffer_func);


    memset(buffer_func, 0, sizeof(buffer_func));

    snprintf(buffer_func,sizeof(buffer_func),"[(%04d) %-50s]: ",line, file);
    impl_printf(buffer_func);
